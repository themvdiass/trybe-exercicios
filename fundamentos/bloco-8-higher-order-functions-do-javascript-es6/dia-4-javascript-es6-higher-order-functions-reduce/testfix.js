...
const sum = () => numbers.filter( (number) => number % 2 === 0).reduce( (maior, comp) => (maior + comp))
...
const reportBetter = (students) => students.map((student) => ({ // reportBetter recebe como parametro estudantes
  name: student.nome, // retorno: estudante.name
  materia: student.materias.reduce(getBestClass).name})); // retorno: estudante.materias.reduce()
